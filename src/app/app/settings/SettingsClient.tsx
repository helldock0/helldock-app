...
const OVERWOLF_UID = process.env.NEXT_PUBLIC_OVERWOLF_UID ?? ''
...
export default function SettingsClient({
  ...
  async function revoke(id: string) {
    if (!confirm('Revoke this token? The Overwolf app using it will stop uploading matches.')) return
  ...
  return (
    <main className="px-6 py-6 max-w-3xl mx-auto">
      ...
      <section className="bg-surface-2 border border-line-strong/40 rounded-2xl p-5 mb-5">
        ...
        <p className="text-sm text-muted-2 mb-4">
          Bearer tokens for the Helldock Overwolf app — used to ingest custom games
          where the host enabled &quot;Hide Match History&quot;. Each teammate
          running the app needs their own token. Plaintext is shown{' '}
          <strong className="text-fg">once</strong>; if lost, revoke + regenerate.
        </p>
        ...
        {genState.kind === 'shown' && (
          <div className="mb-4 rounded-md border border-gold/50 bg-gold/5 p-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <code className="flex-1 min-w-0 bg-surface border border-line-strong rounded-md px-3 py-2 text-xs text-fg font-mono break-all">
                {genState.plaintext}
              </code>
              <button
                type="button"
              ...
              {OVERWOLF_UID && (
                <a
                  href={`overwolf-extension://${OVERWOLF_UID}?token=${encodeURIComponent(genState.plaintext)}&team=${encodeURIComponent(teamSlug)}`}
                  className="px-3 py-2 rounded-md border border-indigo-500/50 bg-indigo-500/10 text-indigo-400 font-semibold text-xs hover:bg-indigo-500/20 transition-colors whitespace-nowrap"
                >
                  Open in Overwolf
                </a>
              )}
        ...
        <details className="mt-5 text-xs text-muted-2">
          <summary className="cursor-pointer hover:text-muted">
            How to install Helldock for Overwolf
          </summary>
          <ol className="mt-2 ml-5 list-decimal space-y-1 leading-relaxed">
            <li>Install <a href="https://www.overwolf.com" target="_blank" rel="noreferrer" className="underline hover:text-muted">Overwolf</a> if you don&apos;t have it</li>
            <li>Download the <code className="font-mono">helldock.opk</code> from the team Drive folder and double-click to install</li>
            <li>Generate a token above, then click <strong className="text-fg">Open in Overwolf</strong> to auto-configure — or paste it manually in the Helldock settings window</li>
            <li>Launch VALORANT — Helldock starts automatically and captures every match ~3 min after it ends</li>
          </ol>
        </details>
      </section>
    </main>
