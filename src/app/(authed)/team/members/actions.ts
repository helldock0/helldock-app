...
export async function inviteTeamMemberAction(formData: FormData) {
  ...
  if (!isOrgAdmin) redirect(`/team/members?error=forbidden`)

  const email = String(formData.get('email') ?? '').trim().toLowerCase()
  const role = String(formData.get('role') ?? '').trim() as (typeof ALLOWED_TEAM_ROLES)[number]
  if (!email) redirect('/team/members?error=email')
  if (!ALLOWED_TEAM_ROLES.includes(role)) redirect('/team/members?error=role')
  ...
  if (error) {
    ...
    redirect(`/team/members?error=db`)
  }
  ...
  redirect('/team/members?invited=1')
}
