...
type TeamCard = { id: string; slug: string; name: string }

export default async function SelectTeamPage() {
  const ctx = await getCurrentUserContext()
  if (!ctx) redirect('/login')

  // Collect teams the user is explicitly a member of
  const membershipTeams: TeamCard[] = ctx.memberships.flatMap((org) =>
    org.teams.map((t) => ({ id: t.teamId, slug: t.teamSlug, name: t.teamName }))
  )

  // Platform admins can browse every team
  let teams: TeamCard[] = membershipTeams
  if (ctx.isPlatformAdmin && membershipTeams.length === 0) {
    const supabase = createClient()
    const { data } = await supabase.from('teams').select('id, slug, name').order('name')
    teams = data ?? []
  }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6 bg-bg">
      <div className="w-full max-w-3xl">
        ...
        {teams.length === 0 ? (
          <div className="rounded-2xl border border-line-strong/40 bg-surface-2 p-10 text-center">
            <p className="text-fg mb-2">You aren&rsquo;t on a team yet.</p>
            <p className="text-muted text-sm">
              Ask your org admin to add you, or wait for an invite email.
            </p>
          </div>
        ) : (
